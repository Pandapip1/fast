...
def _gen_initial_sims(base_dir, initial_struct, trj_obj, n_kids, q_check_obj):
    ...
    for kid in range(n_kids):
        ...
        try:
            ...
            if os.path.exists(output_full):
                ...
                cmds.append("mv " + kid_dir + "/frame0_aligned.xtc " + output_full)
            if os.path.exists(output_masses):
                logging.info("file '%s' exists! skipping move." % output_masses)
            else:
                cmds.append("mv " + kid_dir + "/frame0_masses.xtc " + output_masses)
            if len(cmds) > 0:
                out = tools.run_commands(cmds)
        # give sad-face expression
        except:
            raise MissingData(
                "trajectory from gen %03d and kid %03d was not found." % (gen_num, kid),
...
def _pickle_submit(msm_dir, base_obj, sub_obj, q_check_obj, gen_num, base_name):
    ...
    sub_output = "submissions/" + base_name + "_gen" + ("%03d" % gen_num) + ".out"
    base_pickle_output = (
        "submissions/" + base_name + "_gen" + ("%03d" % gen_num) + ".pkl"
    )
    base_python_output = (
...
def _determine_gen(output_dir, ignore_error=False):
    ...
    trj_gen_nums = np.array([int(n.split("gen")[-1].split("_")[0]) for n in trj_names])
...
def _move_cluster_data(msm_dir, rebuild_num, analysis_obj=None):
    ...
    cmd1 = "mv " + msm_dir + "/data " + old_dir + "/data" + str(rebuild_num) + backup
    cmd2 = (
        "mv "
        + msm_dir
        + "/centers_masses "
...
def push_forward(s, num=0):
    s_out = s.split("\n")
    s_pushed = "\n".join(["".join(itertools.repeat(" ", num)) + l for l in s_out])
    return s_pushed
...
class AdaptiveSampling(base):
    ...
    def __init__(
        ...
        self.save_restart_obj = SaveWrap(centers="restarts", save_routine="restarts")
    ...
    def run(self):
        ...
        if not self.continue_prev:
            ...
            _gen_initial_sims(
                self.output_dir,
                self.initial_state_md,
                self.sim_obj,
                self.n_kids,
                self.q_check_obj_sim,
            )
            ...
            if self.save_state_obj is not None:
                ...
                correct_save = self.save_state_obj.check_save_states(self.msm_dir)
                if not correct_save:
                    raise MissingData("Saving states failed!")
                t_post = time.time()
                logging.info("saving states took %0.4f seconds" % (t_post - t_pre))
        ...
        # rank states, and restart simulations
            ...
            if not os.path.exists(self.output_dir):
                raise DataInvalid(
                    "Can't continue run from output directory that doesn't" + " exist!"
                )
            ...
            if not correct_clust:
                ...
                if gen_num == 0:
                    try:
                        _move_cluster_data(self.msm_dir, rebuild_num, self.analysis_obj)
                    except:
                        pass
                else:
                    _move_cluster_data(self.msm_dir, rebuild_num, self.analysis_obj)
        ...
        self.msm_obj = _prop_msm(self.msm_dir, self.msm_obj)
        ...
        np.save(
            self.msm_dir + "/rankings/states_to_simulate_gen" + str(gen_num) + ".npy",
            new_states,
        )
        ...
        for gen_num in np.arange(gen_num + 1, self.n_gens):
            ...
            _prop_sims(
                self.output_dir, self.sim_obj, gen_num, self.q_check_obj_sim, new_states
            )
            # move trajectories
            logging.info("moving trajectories")
            _move_trjs(gen_dir, self.msm_dir, gen_num, self.n_kids)
            ...
            np.save(
                self.msm_dir
                + "/rankings/states_to_simulate_gen"
                + str(gen_num)
                + ".npy",
                new_states,
            )
